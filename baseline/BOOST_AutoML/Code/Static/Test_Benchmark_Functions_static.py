...
def test_benchmark_function(use_xue=False, kernels=[KernelType.TBD], acquisitions=[AcquisitionType.TBD], benchmarks=None, n_init_points=10, max_iter=100, trial=10):
    if use_xue:
        base_dir = str(_PROJECT_DIR / f'results/results_xue_{datetime.now().strftime("%Y%m%d")}_trial{trial}')
    else:
            ...
            for kernel_type in kernels:
                ...
                for i in range(trial):
                    ...
                    current_trial_results.append({
                        ...
                        'seed': i,
                        'method': 'xue' if use_xue else 'determined',
                        **result
                    })
...
if __name__ == '__main__':
    ...
    test_benchmark_function(use_xue=True, benchmarks=benchmarks, n_init_points=10, max_iter=100, trial=30)
