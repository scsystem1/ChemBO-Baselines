...
from Static._class_for_test_static import TestFunction
...
def test_hpob(use_xue=False, kernels=[KernelType.TBD], acquisitions=[AcquisitionType.TBD], benchmarks=None, n_init_points=10, max_iter=100, trial=10):
    if use_xue:
        base_dir = str(_PROJECT_DIR / f'results/results_HPOB_xue_{datetime.now().strftime("%Y%m%d")}_trial{trial}')
    else:
        base_dir = str(_PROJECT_DIR / f'results/results_HPOB_{datetime.now().strftime("%Y%m%d")}')
            ...
            for kernel_type in kernels:
                ...
                for i in range(trial):
                    ...
                    current_trial_results.append({
                        'objective': f'{data_num}_{dim}D',
                        'seed': i,
                        'method': 'xue' if use_xue else None,
                        **result
                    })
...
if __name__ == '__main__':
    ...
    for benchmark in benchmark_list:
        ...
        test_hpob(use_xue=True, benchmarks=benchmarks, n_init_points=10, max_iter=100, trial=30)
