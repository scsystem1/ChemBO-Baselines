...
from BayesianOptimization_for_Best_Utility import BayesianOptimizer
    ...
    def optimize_recommend_adaptive(self):
        ...
        for iter in pbar:
            ...
            for kernel_candidate in kernel_candidates:
                ...
                next_x_cand, next_y_cand, next_x_idx_cand, max_acq_value_cand = self.get_next_point(train_x=self.train_x, train_y=self.train_y, filtered_candidate_x=self.filtered_candidate_x, filtered_candidate_y=self.filtered_candidate_y, kernel_type=kernel_candidate, acquisition_type=self.acquisition_type, objective=self.objective)
                next_X.append(next_x_cand)
                next_Y.append(next_y_cand)
                next_X_idx.append(next_x_idx_cand)
                max_acq_value.append(max_acq_value_cand)

            # update train_x and train_y
            best_utility_idx = torch.argmax(torch.tensor(max_acq_value))
            next_x = next_X[best_utility_idx]
            next_y = next_Y[best_utility_idx]
            next_x_idx = next_X_idx[best_utility_idx]
            self.kernel_type = kernel_candidates[best_utility_idx]
            kernel_log.append([iter] + [self.kernel_type.value])
