...
def test_hpob(use_boost=False, kernels=[KernelType.TBD], acquisitions=[AcquisitionType.TBD], benchmarks=None, n_init_points=10, max_iter=100, trial=10):
    if use_boost:
        base_dir = str(_PROJECT_DIR / f'results/results_HPOB_boost_{datetime.now().strftime("%Y%m%d")}')
    else:
...
if __name__ == '__main__':
    ...
    for benchmark in benchmark_list:
        ...
        use_boost = True
        if use_boost:
            test_hpob(use_boost=True, benchmarks=benchmarks, n_init_points=10, max_iter=100, trial=30)
        else:
