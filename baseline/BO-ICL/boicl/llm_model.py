...
from .kimi_utils import build_kimi_client, is_kimi_model, resolve_kimi_model_name
...
def get_llm(
    ...
    **kwargs,
    ...
    model_name = resolve_kimi_model_name(model_name)
    ...
    if is_kimi_model(model_name):
        return KimiCompatibleLLM(**kwargs)
...
class KimiCompatibleLLM(LLM):
    def create_llm(self):
        return build_kimi_client()

    def _create_messages(self, query: str, system_message: str = ""):
        messages = []
        if system_message:
        ...
        for query in query_list:
            ...
            for _ in range(self.n):
                completion = self.llm.chat.completions.create(
                    model=resolve_kimi_model_name(self.model_name),
                    messages=self._create_messages(query, system_message),
                ...
                if getattr(completion, "usage", None) is not None:
                    token_usage += getattr(completion.usage, "total_tokens", 0) or 0
    ...
    def parse_response(self, generations):
        ...
        for gen in generations:
            try:
                values.append(float(truncate(gen)))
            except ValueError:
                continue
