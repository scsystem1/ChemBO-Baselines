...
from abc import ABC
import os
os.environ.setdefault("OMP_NUM_THREADS", "100")


def _to_1d_int_numpy(values):
...
class BaseDataModule(pl.LightningDataModule, ABC):
    ...
    def split_data(self):
        ...
        init_indexes = _to_1d_int_numpy(init_indexes)
        ...
        self.heldout_indices = _to_1d_int_numpy(heldout_df_indices)

        sorted_indices = torch.argsort(self.heldout_y.view(-1))
        sorted_indices_np = _to_1d_int_numpy(sorted_indices)

        self.heldout_x = self.heldout_x[sorted_indices_np]
        self.heldout_y = self.heldout_y[sorted_indices_np]
        self.heldout_indices = self.heldout_indices[sorted_indices_np]
    ...
    def update_results(self, experiment_results, experiment_indices):
        ...
        self.train_indexes = np.concatenate(
            [
                _to_1d_int_numpy(self.train_indexes),
                _to_1d_int_numpy(experiment_indices),
            ]
        )
