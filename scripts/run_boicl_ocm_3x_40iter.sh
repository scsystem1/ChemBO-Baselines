...
echo "[BO-ICL][OCM] Starting run with model=${BOICL_LLM_MODEL}, embed_model=${BOICL_EMBED_MODEL}, budget=40, trials=3"
conda run --no-capture-output -n boicl python "${ROOT_DIR}/baseline/BO-ICL/run_tabular_experiment.py" \
  --dataset ocm \
  --trials 3 \
  --total-budget 40 \
  --model "${BOICL_LLM_MODEL}" \
  --output-dir "${ROOT_DIR}/outputs/baseline_runs/boicl/ocm"
