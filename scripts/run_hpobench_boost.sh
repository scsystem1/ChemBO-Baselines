...
print_config() {
  ...
  echo "[HPOBench-BOOST] boost_dir=${OUT_ROOT}/${BOOST_OUTPUT_SUBDIR}/<problem>"
}
...
results_path = out_root / output_subdir / problem / f'{problem}_boost_results.npz'
completed = 0
recorded = 0
has_incomplete = 0
...
" "${OUT_ROOT}" "${BOOST_OUTPUT_SUBDIR}" "${problem}" "${TOTAL_BUDGET}"
}
...
run_problem() {
  ...
  local data_path resume_info existing_completed existing_recorded has_incomplete remaining_trials
  data_path="$(problem_data_path "${problem}")"
