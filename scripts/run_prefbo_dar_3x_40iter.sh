...
echo "[PrefBO][DAR] Starting run with model=${PREFBO_LLM_MODEL}, budget=${BUDGET}, trials=${TRIALS}, seed_start=${SEED_START}, seed_end=$((SEED_START + TRIALS - 1)), cpu_cap=${CPU_THREAD_CAP}, survey_questions=first_2000_from_file"
"${CPU_AFFINITY_PREFIX[@]}" conda run --no-capture-output -n prefbo python "${ROOT_DIR}/baseline/Pref-BO/run_tabular_preference_bo.py" \
  ...
  --total-budget "${BUDGET}" \
  --questions-file "${ROOT_DIR}/baseline/Pref-BO/questions/dar_questions_seed0.npy" \
  --max-questions 2000 \
  --max-cpu-threads "${CPU_THREAD_CAP}" \
  --output-dir "${ROOT_DIR}/outputs/baseline_runs/prefbo/dar"
