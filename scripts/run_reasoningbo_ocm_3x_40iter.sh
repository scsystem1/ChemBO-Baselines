...
echo "[ReasoningBO][OCM] Starting run with model=${REASONINGBO_LLM_MODEL}, total_budget=40, reasoning_batch_size=3, trials=3"
conda run --no-capture-output -n reasoning_bo python "${ROOT_DIR}/baseline/Reasoning-BO/run_tabular_reasoning_bo.py" \
  ...
  --output-dir "${ROOT_DIR}/outputs/baseline_runs/reasoning_bo/ocm"
